...
def train_bluffing_baseline(episodes=10000):
    ...
    for ep in range(1, episodes + 1):
        ...
        for name in env.agent_iter():
            ...
            if name == "player_0":
                ...
                deception_rewards.append(compute_deception_reward(bluff_score=bluff_score, action=action, bluff_reward=3))
        ...
        # Count total bluff attempts and successes
        ...
        writer.add_scalar("Bluff/SuccessRate", percent_successful_bluff, ep)
        #print(f"[Ep {ep}] Bluff Attempts: {bluff_attempts}, Successes: {successful_bluffs} ({percent_successful_bluff:.1f}%)")

        total_bluff_attempts += bluff_attempts
        total_successful_bluffs += successful_bluffs

        if states_this_game:
            bluff_avg = np.mean(bluff_scores)
            writer.add_scalar("Bluff/AverageScore", bluff_avg, ep)
            #print(f"[Ep {ep}] Avg Bluff Score: {bluff_avg:.3f}")
    ...
    if all_bluff_scores:
        ...
        print(f"\n=== Bluff Statistics ===")
        print(f"Average Reward Across All Steps: {avg_bluff_score:.3f}")
        non_zero_bluffs = [b for b in all_bluff_scores if b > 0]
        avg_bluff_score = np.mean(non_zero_bluffs) if non_zero_bluffs else 0.0
