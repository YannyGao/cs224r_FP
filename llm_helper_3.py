...
def train_bluffing_baseline(episodes=10000):
    ...
    agent = BaselineAgent()
    # agent.policy.load_state_dict(torch.load("checkpoints/main_agent_ep5000.pt"))
    # print("Loaded checkpoint from episode 5000")
    ...
    for ep in range(1, episodes + 1):
        ...
        # if not won_game and (previous_feedback != "") and stats_after and stats_before:
        #     adherence_score = get_adherence_score_from_llm(previous_feedback, stats_before, stats_after)
        #     print(adherence_score)
        # else:
        #     adherence_score = None
        # if won_game:
        #     print(f"won game in ep {ep}")
    
        
        # if not won_game and adherence_score and combined_rewards:
        #     combined_rewards[-1] += adherence_score*10
        ...
        if not np.isnan(accuracy):
            accuracies_by_type[opponent_type].append(accuracy)
            writer.add_scalar(f"OpponentModel/Accuracy_{opponent_type}", accuracy, ep)
        ...
        # Generate feedback with corrected stats
        feedback_text = get_gpt_feedback(main_agent_agg_stats, opponent_agg_stats)
        print(f"GPT Feedback: {feedback_text}")
